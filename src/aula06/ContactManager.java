...
public class ContactManager {
    ...
    private static boolean personInContactsHandler(Person person) {
        List<Contact> contactsByPerson = contacts.stream().filter(contact -> contact.getPerson() == person)
    ...
    private static Contact getContactByName(String name) {
        List<Contact> contactsByName = contacts.stream().filter(contact -> contact.getPerson().getName().equals(name))
                .collect(Collectors.toList());
    ...
    private static Contact getContactByNumber(int number) {
        List<Contact> contactsByName = contacts.stream().filter(contact -> contact.getPhone() == number)
                .collect(Collectors.toList());
        if (contactsByName.isEmpty()) {
            return null;
        }
